...
func (k Keeper) RouteLockedBalancerToConcentratedMigration(ctx sdk.Context, sender sdk.AccAddress, lockId uint64, sharesToMigrate sdk.Coin, tokenOutMins sdk.Coins) (positionId uint64, amount0, amount1 sdk.Int, liquidity sdk.Dec, joinTime time.Time, poolIdLeaving, poolIdEntering, gammLockId, concentratedLockId uint64, err error) {
	...
	poolIdLeaving, poolIdEntering, preMigrationLock, remainingLockTime, synthLockBeforeMigration, isSuperfluidBonded, isSuperfluidUnbonding, err := k.prepareMigration(ctx, sender, lockId, sharesToMigrate)
	if err != nil {
	...
	if isSuperfluidBonded {
		...
		positionId, amount0, amount1, liquidity, joinTime, gammLockId, concentratedLockId, err = k.migrateSuperfluidBondedBalancerToConcentrated(ctx, sender, poolIdLeaving, poolIdEntering, preMigrationLock, lockId, sharesToMigrate, synthLockBeforeMigration[0].SynthDenom, remainingLockTime, tokenOutMins)
	} else if isSuperfluidUnbonding {
		// Migration logic for superfluid unbonding locks
		positionId, amount0, amount1, liquidity, joinTime, gammLockId, concentratedLockId, err = k.migrateSuperfluidUnbondingBalancerToConcentrated(ctx, sender, poolIdLeaving, poolIdEntering, preMigrationLock, sharesToMigrate, synthLockBeforeMigration[0].SynthDenom, remainingLockTime, tokenOutMins)
	} else if !isSuperfluidBonded && !isSuperfluidUnbonding && len(synthLockBeforeMigration) == 0 {
		// Migration logic for non-superfluid locks
		positionId, amount0, amount1, liquidity, joinTime, gammLockId, concentratedLockId, err = k.migrateNonSuperfluidLockBalancerToConcentrated(ctx, sender, poolIdLeaving, poolIdEntering, preMigrationLock, sharesToMigrate, remainingLockTime, tokenOutMins)
	} else {
...
func (k Keeper) migrateSuperfluidUnbondingBalancerToConcentrated(
	ctx sdk.Context,
	sender sdk.AccAddress,
...
func (k Keeper) migrateNonSuperfluidLockBalancerToConcentrated(
	ctx sdk.Context,
	sender sdk.AccAddress,
	poolIdLeaving, poolIdEntering uint64,
	preMigrationLock *lockuptypes.PeriodLock,
...
func (k Keeper) prepareMigration(ctx sdk.Context,
	sender sdk.AccAddress,
	lockId uint64,
	sharesToMigrate sdk.Coin) (poolIdLeaving, poolIdEntering uint64,
	preMigrationLock *lockuptypes.PeriodLock,
	remainingLockTime time.Duration,
	synthLockBeforeMigration []lockuptypes.SyntheticLock,
	isSuperfluidBonded, isSuperfluidUnbonding bool, err error) {
	...
	if err != nil {
		return 0, 0, &lockuptypes.PeriodLock{}, 0, nil, false, false, err
	}
	...
	if isSuperfluidBonded && isSuperfluidUnbonding {
		...
		return 0, 0, &lockuptypes.PeriodLock{}, 0, nil, false, false, fmt.Errorf("synthetic lock %d must be either superfluid delegated or superfluid undelegating, not both", lockId)
	}

	return poolIdLeaving, poolIdEntering, preMigrationLock, remainingLockTime, synthLockBeforeMigration, isSuperfluidBonded, isSuperfluidUnbonding, nil
}
