...
func (suite *KeeperTestSuite) TestMigrateSuperfluidBondedBalancerToConcentrated() {
	...
	for name, tc := range testCases {
		suite.Run(name, func() {
			...
			poolIdLeaving, poolIdEntering, preMigrationLock, remainingLockTime, synthLockBeforeMigration, _, _, err := superfluidKeeper.PrepareMigration(ctx, poolJoinAcc, originalGammLockId, coinsToMigrate)
			suite.Require().NoError(err)
			...
			// System under test.
			positionId, amount0, amount1, _, _, newGammLockId, concentratedLockId, err := superfluidKeeper.MigrateSuperfluidBondedBalancerToConcentrated(ctx, poolJoinAcc, poolIdLeaving, poolIdEntering, preMigrationLock, originalGammLockId, coinsToMigrate, synthLockBeforeMigration[0].SynthDenom, remainingLockTime, tc.tokenOutMins)
			if tc.expectedError != nil {
				suite.Require().Error(err)
...
func (suite *KeeperTestSuite) TestMigrateSuperfluidUnbondingBalancerToConcentrated() {
	...
	for name, tc := range testCases {
		suite.Run(name, func() {
			...
			// System under test.
			positionId, amount0, amount1, _, _, newGammLockId, concentratedLockId, err := superfluidKeeper.MigrateSuperfluidUnbondingBalancerToConcentrated(ctx, poolJoinAcc, poolIdLeaving, poolIdEntering, preMigrationLock, coinsToMigrate, synthLockBeforeMigration[0].SynthDenom, remainingLockTime, tc.tokenOutMins)
			if tc.expectedError != nil {
				suite.Require().Error(err)
				suite.Require().ErrorContains(err, tc.expectedError.Error())
				return
...
func (suite *KeeperTestSuite) TestMigrateNonSuperfluidLockBalancerToConcentrated() {
	...
	for name, tc := range testCases {
		suite.Run(name, func() {
			...
			// PrepareMigration is called via the migration message router and is always run prior to the migration itself
			poolIdLeaving, poolIdEntering, preMigrationLock, remainingLockTime, _, _, _, err := superfluidKeeper.PrepareMigration(ctx, poolJoinAcc, originalGammLockId, coinsToMigrate)
			suite.Require().NoError(err)
			...
			// System under test.
			positionId, amount0, amount1, _, _, newGammLockId, concentratedLockId, err := superfluidKeeper.MigrateNonSuperfluidLockBalancerToConcentrated(ctx, poolJoinAcc, poolIdLeaving, poolIdEntering, preMigrationLock, coinsToMigrate, remainingLockTime, tc.tokenOutMins)
			if tc.expectedError != nil {
				suite.Require().Error(err)
				suite.Require().ErrorContains(err, tc.expectedError.Error())
				return
